...
class TestProviderFactory(unittest.TestCase):
    ...
    def test_get_provider_gemini(self):
        ...
        with patch("src.brain.providers.gemini.genai.configure"), patch(
            "src.brain.providers.gemini.genai.GenerativeModel"
        ):
    ...
    def test_get_provider_passes_kwargs(self):
        ...
        with patch("src.brain.providers.openai.OpenAI"):
            provider = get_provider(
                "openai", api_key="test-key", model="gpt-3.5-turbo", custom_param="value"
            )
